...
Figure~\ref{fig:evidencegaps} shows the resulting gap: task-requirement coverage
...
\input{figures/evidence_gaps_float}
...
within-task contrast used by GRPO. The task-weighted rates were 74.5\%,
41.9\%, and 37.2\% on $\tau^2$, BFCL, and ACE. These checks distinguish
...
evaluation outcomes.
\todo{Insert Codex per-iteration results and the learning-signal comparison.}
...
the required branch was absent (Appendix~\ref{app:implementationaudit}).
